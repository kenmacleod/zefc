...
5. **← We are here** — Growable selector registry, vtable growth, and load-time call-site selector patching (replace smoke `zefc_slot_*` scaffolding). In-process `module_load` of hand-compiled packages already exercises load sequencing (`load1`, `load4`); next is the C++-cost dispatch ABI and richer package/load coverage.
