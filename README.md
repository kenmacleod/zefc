...
5. **Dispatch ABI** — Growable registry, `VTable` growth, `ZEFC_SITE` cells (`patch1`); NaN-box Doubles; closed-world `ZEFC_SEL_*` immediates for known names (`nbody`). **← Next:** reloc/text-imm patch for late selectors; broader ScriptBench / compiler.
