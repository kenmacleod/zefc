...
5. **Dispatch ABI** — Growable registry, `VTable` growth, `ZEFC_SITE` cells (`patch1`); shared `zefc_slot_*` removed. Double is Zef-style NaN-box immediate + send short-circuit; ScriptBench `nbody` smoke green. **← Next:** instruction-immediate selectors (true C++-like hot path).
